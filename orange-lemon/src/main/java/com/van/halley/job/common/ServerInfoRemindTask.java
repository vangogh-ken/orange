...
import com.van.halley.util.EmailUtils;
import com.van.halley.util.PropertiesUtils;
import com.van.service.ServerInfoService;

/**
 * Spring调度，指定时间执行 利用了spring 3 中使用注解的方式来进行任务调度。
 */
@Component
public class ServerInfoRemindTask {
	@Autowired
	private ServerInfoService serverInfoService;

	public static void main(String[] args) {
		ServerInfoRemindTask action = new ServerInfoRemindTask();
		try {
			action.action();
		} catch (Exception e) {
	...
	public void action() throws Exception {
		...
		if (null != prop) {
			String data = null;
			try {
				Iterator<Entry<Object, Object>> itr = prop.entrySet()
						.iterator();
				while (itr.hasNext()) {
					Entry<Object, Object> e = (Entry<Object, Object>) itr
							.next();
				...
				EmailUtils.sendMail(prop.getProperty("fromEmail"),
						prop.getProperty("toEmail"),
						prop.getProperty("emailName"),
						prop.getProperty("emailPassword"), "警报！警报！", data);
			} catch (Exception e) {
				EmailUtils.sendMail(prop.getProperty("fromEmail"),
						prop.getProperty("toEmail"),
						prop.getProperty("emailName"),
						prop.getProperty("emailPassword"), "执行异常出错",
						e.toString());
			}
		}
		/****/
	...
	public void task() throws Exception {
		System.out.println(new Date().toLocaleString() + " task 定时任务执行器执行中...");

		ServerStatus status = Common.getServerStatus();
		String cpuUsage = status.getCpuUsage();// CPU使用率
		String serverUsage = Common.fromUsage(status.getUsedMem(),
				status.getTotalMem());// 系统内存使用率
		String JvmUsage = Common.fromUsage(status.getJvmFreeMem(),
				status.getJvmTotalMem());// 计算ＪＶＭ内存使用率
		Properties prop = PropertiesUtils.getProperties();
		...
		String centent = "当前时间是：" + Common.fromDateH() + "<br/><br/>"
				...
				+ "</tbody>" + "</table>";
		if (!Common.isEmpty(cpubool) || !Common.isEmpty(jvmbool)
				|| !Common.isEmpty(rambool)) {
			try {
				EmailUtils.sendMail(prop.getProperty("fromEmail"), email,
						prop.getProperty("emailName"),
	...
	public String task(String username, String pass) throws Exception {
		...
		try {
			url = new URL("http://www.jhost.cn/os/loginCheck.action?user="
					+ username + "&pass=" + pass + "&module=login");
			...
			if (httpConn.getResponseCode() == 200) {
				...
				data += "jhost 于" + Common.fromDateH() + "　返回服务器状态："
						+ httpConn.getResponseCode() + "　连接失败！！";
			}
