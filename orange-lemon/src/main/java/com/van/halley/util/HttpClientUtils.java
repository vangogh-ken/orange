...
public class HttpClientUtils {
	...
	public Set<String> toHtmlSet(String urlStr) {
		...
		if (html != null && !"".equals(html)) {
			...
			for (LinkTag linkTag : linkTags) {
				if (!Common.isEmpty(linkTag.getLink())
						&& linkTag.getLink().indexOf("v_show/id_") > -1) {
	...
	public List<String> getLinkTagsContent(String urlStr) throws Exception {
		...
		urlStr = "http://www.flvcd.com/parse.php?kw=" + urlStr
				+ "&flag=one&format=high";
		...
		if (html != null) {
			if (html.indexOf("：</strong>") > -1) {
				...
				// 取得视频连接内容
				String content = parseTag3(html, TableColumn.class, "class",
						"mn STYLE4").getStringText();
	...
	public List<Map<String, String>> getYueShiMusic() {
		...
		if (html != null && !"".equals(html)) {
			...
			for (ImageTag imageTag : imageTags) {
				if (sun < 10) {
					if (map.get(imageTag.getAttribute("alt")) != null
							&& !"".equals(imageTag.getAttribute("alt"))) {
						mapVideo = new HashMap<String, String>();
						mapVideo.put("videoTypeId", "1");
						mapVideo.put("videoName",
								"" + imageTag.getAttribute("alt"));
						mapVideo.put("videoSmallImagesUrl",
								"" + imageTag.getAttribute("data-src"));
						mapVideo.put("videoUrl",
								"" + map.get(imageTag.getAttribute("alt")));
						mapVideo.put(
								"videoLable",
								""
										+ imageTag.getAttribute("alt")
												.substring(0, 2));
						mapVideo.put("videoProfile",
								"" + imageTag.getAttribute("alt"));
	...
	public List<Map<String, String>> getYouKuFunny() {
		...
		String html = Common
				.getInputHtmlUTF8("http://fun.youku.com/index/focus");
		...
		if (html != null && !"".equals(html)) {
			...
			for (ImageTag imageTag : imageTags) {
				if (sun < 10) {
					if (map.get(imageTag.getAttribute("alt").trim()) != null
							&& !"".equals(imageTag.getAttribute("alt").trim())) {
						mapVideo = new HashMap<String, String>();
						mapVideo.put("videoTypeId", "2");
						mapVideo.put("videoName",
								"" + imageTag.getAttribute("alt").trim());
						mapVideo.put("videoSmallImagesUrl", ""
								+ imageTag.getImageURL().trim());
						mapVideo.put(
								"videoUrl",
								""
										+ map.get(imageTag.getAttribute("alt")
												.trim()));
						mapVideo.put("videoLable",
								""
										+ imageTag.getAttribute("alt").trim()
												.substring(0, 2));
						mapVideo.put("videoProfile", ""
	...
	public List<Map<String, String>> getXinLangNews() {
		...
		String html = Common
				.getInputHtmlGBK("http://v.sina.com.cn/v/paike.html");
		...
		if (html != null && !"".equals(html)) {
			...
			for (ImageTag imageTag : imageTags) {
				if (imageTag.getAttribute("alt").trim() != null
						&& !"".equals(imageTag.getAttribute("alt"))) {
					map.put(imageTag.getAttribute("alt").trim(),
			...
			Map<String, String> mapVideo = null;
			for (LinkTag linkTag : linkTags) {
				if (map.get(linkTag.getLinkText()) != null
						&& !"".equals(map.get(linkTag.getLinkText()))) {
					mapVideo = new HashMap<String, String>();
					mapVideo.put("videoTypeId", "3");
					mapVideo.put("videoName", "" + linkTag.getLinkText());
					mapVideo.put("videoSmallImagesUrl",
							"" + map.get(linkTag.getLinkText()));
					mapVideo.put("videoUrl", "" + linkTag.getLink());
					mapVideo.put("videoLable", ""
							+ linkTag.getLinkText().substring(0, 2));
		...
		if (html != null && !"".equals(html)) {
			...
			for (LinkTag linkTag : linkTags) {
				if (linkTag.getLink().trim().indexOf("ent") > -1) {
					map.put(linkTag.getLinkText().trim(), linkTag.getLink()
							.trim());
				}
	...
	public static <T extends TagNode> List<T> parseTags(String html,
			final Class<T> tagType, final String attrbuteName,
			final String attrbutValue) {
		try {
			Parser parser = new Parser();
			parser.setInputHTML(html);
	 ...
	 * @return 第一个标签
	 */
	public static <T extends TagNode> T parseTag0(String html,
			final Class<T> tagType, final String attrbuteName,
			final String attrbutValue) {
	 ...
	 * @return 第3个标签
	 */
	public static <T extends TagNode> T parseTag3(String html,
			final Class<T> tagType, final String attrbuteName,
			final String attrbutValue) {
		List<T> tags = parseTags(html, tagType, attrbuteName, attrbutValue);
