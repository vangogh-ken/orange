...
import com.van.halley.out.web.ServerInfoController;
...
public class Common {
	...
	// 磁盘读写初始数据 用于计算读写速率
	/**
	 * private static Map<String, String> diskWritesAndReadsOnInit = new
	 * HashMap<String, String>(); private static long initTime; static {
	 * initTime = System.currentTimeMillis(); resetClasspath(); Sigar sigar =
	 * null; try {
	 * 
	 * sigar = new Sigar(); FileSystem fslist[] = sigar.getFileSystemList();
	 * FileSystemUsage usage = null; for (int i = 0; i < fslist.length; i++) {
	 * FileSystem fs = fslist[i]; if (fs.getType() != 2) continue; usage =
	 * sigar.getFileSystemUsage(fs.getDirName());
	 * diskWritesAndReadsOnInit.put(fs.getDevName(), usage.getDiskReadBytes() +
	 * "|" + usage.getDiskWriteBytes()); } } catch (Exception e) { } finally {
	...
	private static void resetClasspath() {
		...
		String classpath = ServerInfoController.class.getResource("/")
				.getPath();
		System.setProperty("java.library.path", classpath + File.separator
				+ "sigar" + File.pathSeparator + libPath);
	}
	...
	public static ServerStatus getServerStatus() throws Exception {
		...
		/*
		 * Sigar sigar = new Sigar(); getServerCpuInfo(sigar, status);
		 * getServerDiskInfo(sigar, status); getServerMemoryInfo(sigar, status);
		 */
	 ...
	 * try { CpuInfo infos[] = sigar.getCpuInfoList(); CpuPerc cpuList[] =
	 * sigar.getCpuPercList(); double totalUse = 0L; for (int i = 0; i <
	 * infos.length; i++) { CpuPerc perc = cpuList[i]; CpuInfoVo cpuInfo = new
	 * CpuInfoVo(); cpuInfo.setId(infos[i].hashCode() + "");
	 ...
	 * status.getCpuInfos().add(cpuInfo); totalUse += perc.getCombined(); }
	 * String cpuu = CpuPerc.format(totalUse / status.getCpuInfos().size());
	 * cpuu = cpuu.substring(0,cpuu.length()-1); status.setCpuUsage(cpuu); }
	...
	public static Map<String, Object> modifySer(HttpServletRequest request,
	 ...
	 * { try { Mem mem = sigar.getMem(); status.setTotalMem(mem.getTotal() /
	 * (1024 * 1024)); status.setUsedMem(mem.getUsed() / (1024 * 1024));
	 * status.setFreeMem(mem.getFree() / (1024 * 1024)); // 交换区 Swap swap =
	 * sigar.getSwap(); status.setTotalSwap(swap.getTotal() / (1024 * 1024));
	 * status.setUsedSwap(swap.getUsed() / (1024 * 1024));
	 * status.setFreeSwap(swap.getFree() / (1024 * 1024)); } catch (Exception e)
	 * {
	 * 
	 * } }
	 **/
	 ...
	 * try { FileSystem fslist[] = sigar.getFileSystemList(); FileSystemUsage
	 * usage = null; for (int i = 0; i < fslist.length; i++) { FileSystem fs =
	 * fslist[i]; switch (fs.getType()) { case 0: // TYPE_UNKNOWN ：未知 case 1: //
	 * TYPE_NONE case 3:// TYPE_NETWORK ：网络 case 4:// TYPE_RAM_DISK ：闪存 case
	 * 5:// TYPE_CDROM ：光驱 case 6:// TYPE_SWAP ：页面交换 break; case 2: //
	 * TYPE_LOCAL_DISK : 本地硬盘 DiskInfoVo disk = new DiskInfoVo();
	 * disk.setDevName(fs.getDevName()); disk.setDirName(fs.getDirName()); usage
	 * = sigar.getFileSystemUsage(fs.getDirName());
	 ...
	 * 
	 * String val = diskWritesAndReadsOnInit.get(fs.getDevName()); if (val !=
	 * null) { long timePeriod = (System.currentTimeMillis() - initTime) / 1000;
	 * long origRead = Long.parseLong(val.split("\\|")[0]); long origWrite =
	 * Long.parseLong(val.split("\\|")[1]);
