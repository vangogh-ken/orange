	...
	if ($("#basisSubstanceForm").valid()) {
		...
		$.post('../bpm/workspance-task-done-complete-task.do', $('#basisSubstanceForm').serialize() + '&' + activityId, function(data, status) {
			if(data == 'success') {
				...
				alert('发送任务失败，请确实数据是否填写正确及下个任务的权限人！');
				box.modal('hide');
			}
		});
	}else{
 ...
 * 通过微信提交完成任务
 */
function completeTaskWithCertain(activityId){
	if ($("#basisSubstanceForm").valid()) {
		$.post('../wx/workspance-task-done-complete-task.do', $('#basisSubstanceForm').serialize() + '&activityId=' + activityId, function(data, status) {
			if(data == 'success') {
				alert('发送成功，请关闭页面！');
				//window.close();
			}else {
