			...
			<section id="m-main-input" style="margin-left:10px;margin-right:0;min-width: 1000px;min-height: 700px;max-height: 1000px;">
				<article class="m-widget">
					...
					<div class="row">
						...
						<div class="col-md-9">
							<form id="freightOrderForm" method="post" action="fre-order-save-service.do" enctype="multipart/form-data" onsubmit="return false" class="form-horizontal" >
									...
									<tbody>
										...
										size="40" minlength="2" maxlength="50" class="form-control" readonly="readonly">
										  ...
										  <tr>
											...
											<td>
												<select id="orderScope" name="orderScope" class="form-control required" 
													...
													<option value="销售自揽" <c:if test="${item.orderScope == '销售自揽'}">selected</c:if> >销售自揽</option>
													<option value="公司指定" <c:if test="${item.orderScope == '公司指定'}">selected</c:if> >公司指定</option>
													<option value="客服自揽" <c:if test="${item.orderScope == '客服自揽'}">selected</c:if> >客服自揽</option>
													<option value="客服自揽(项目)" <c:if test="${item.orderScope == '客服自揽(项目)'}">selected</c:if> >客服自揽(项目)</option>
												</select>
												<!--  
										 ...
										 <tr>
											...
											<td>
												...
												<input id="muiltFile" type="file" name="muiltFile" value="${item.commission}" 
													size="40" minlength="2" maxlength="64" class="form-control input-xsmall <c:if test="${item.commission == null}">required</c:if>" >
												</c:if>
											</td>
										</tr>
										<tr>
	...
	<div id="actionFieldModal" class="modal fade" aria-hidden="true">
		<div class="modal-content">
			...
			<div class="modal-body">
				...
				<form id="orderBoxSelectForm" class="m-form-blank"></form>
				</article>
			</div>
			<div class="modal-footer">
				<button type="button" class="btn btn-sm default" data-dismiss="modal">关闭</button>
			...
			if($('#freightOrderForm #id').val() == undefined || $('#freightOrderForm #id').val() == ''){
				...
				$.ajax({
					...
					success:function(data){
						...
						//html += '<td><input id="blNo" name="blNo" class="form-control"></td>';
						html += '</tr>'
