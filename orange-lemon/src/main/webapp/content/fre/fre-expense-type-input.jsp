				  ...
				  <div class="col-md-12">
				  	<div class="portlet box red">
						<div class="portlet-title">
							<div class="caption"><i class="fa fa-align-justify"></i>费用类型信息</div>
						</div>
						<div class="portlet-body form">
						<!-- BEGIN FORM-->
							<form id="editDomainForm" method="post" action="fre-expense-type-save.do?operationMode=STORE" class="form-horizontal">
								 ...
								 <div class="form-body">
									<table class="table-input" style="margin-left:20%;width:60%">
										<thead>
											<tr><th colspan="6">费用类型信息</th></tr>
										</thead>
										<tbody>
											<tr>
												<td>
													<label class="td-label" for="typeName">费用名称</label>
												</td>
												<td>
												...
												<td>
													<label class="td-label" for="revenue">是否为营收费用</label>
												</td>
												<td>
													<select id="status" name="revenue" class="form-control required">
														<option value="T"<c:if test="${item.revenue == 'T'}">selected</c:if> >是</option>
														<option value="F"<c:if test="${item.revenue == 'F'}">selected</c:if> >否</option>
													</select>
												</td>
											</tr>
											<tr>
    ...
    $(function() {
        $("#editDomainForm").validate({
            ...
            rules: {
            	typeName: {
   	                remote: {
   	                    url: 'fre-expense-type-check-typename.do',
   	                    data: {
   	                        <c:if test="${item.id != null}">
   	                        id: function() {
   	                            return $('#id').val();
