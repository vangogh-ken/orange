...
<body>
			...
			<section id="m-main-input" style="margin-left:10px;margin-right:0;min-width: 1000px;min-height: 700px;max-height: 1000px;">
				<article class="m-widget">
					<header style="height: 35px; margin-top: 0px; margin-bottom: 5px;padding-top:2px;border: solid #858585 1px;">
							...
							<!-- 客服 -->
							<sec:authorize ifAnyGranted="ROLE_FRE-ORDER-INPUT-TO-ORDER-AUDIT">
								<c:if test="${item.orderStatus == '未提交' or item.orderStatus == '已退回' or item.orderStatus == '已追回'}">
								<button id="toOrderAudit" class="btn btn-sm green" style="float: left;margin-right: 10px;">
									提交订单
								</button>
								<button id="addRequire" class="btn btn-sm red" style="float: left;margin-right: 10px;">
									添加箱需
								</button>
								</c:if>
							</sec:authorize>
							
							<sec:authorize ifAnyGranted="ROLE_FRE-ORDER-INPUT-TO-ORDER-RECOVER">
								<c:if test="${item.orderStatus == '审核中' or item.orderStatus == '已审核'}">
								<button id="toOrderRecover" class="btn btn-sm green" style="float: left;margin-right: 10px;">
									追回订单
								</button>
								</c:if>
							</sec:authorize>
							...
							<c:if test="${item.orderStatus == '审核中' or item.orderStatus == '锁定中'}">
								<button class="btn btn-sm green" id="doneOrderAudit" style="float: left;margin-right: 10px;">
								审核订单</button>
								<button class="btn btn-sm green" id="backOrderAudit"style="float: left;margin-right: 10px;">
								退回订单</button>
							</c:if>
							</sec:authorize>
							
							<sec:authorize ifAnyGranted="ROLE_FRE-ORDER-INPUT-SAVE">
							<c:if test="${item.orderStatus == null or item.orderStatus == '未提交' or item.orderStatus == '已退回' or item.orderStatus == '已追回'}">
								<button id="submitButton" onclick="$('#freightOrderForm').submit();" 
									...
									<table id="treeTable" class="table-input">
										<thead>
											<tr>
												<th colspan="2">
													<!-- 客服 -->
													<sec:authorize ifAnyGranted="ROLE_FRE-ORDER-INPUT-MANAGE-MAINTAIN-SALE" >
													<c:if test="${item.orderStatus == '未提交' or item.orderStatus == '已退回' or item.orderStatus == '已追回'}">
													<span style="font-size: 14px;">操作&nbsp;</span>
													<i class="fa fa-plus" onclick="addMaintain();" title="添加操作"></i><span>&nbsp;</span>
													<i class="fa fa-times" onclick="removeMaintain();" title="删除操作"></i><span>&nbsp;</span>
													</c:if>
													</sec:authorize>
													&nbsp;&nbsp;
													...
													<!-- 客服 -->
													<c:if test="${item.orderStatus == '未提交' or item.orderStatus == '已退回' or item.orderStatus == '已追回'}">
													<sec:authorize ifAnyGranted="ROLE_FRE-ORDER-INPUT-MANAGE-ACTION-SALE" >
													<span style="font-size: 14px;">动作&nbsp;</span>
													<i class="fa fa-plus" onclick="addAction();" title="添加动作"></i><span>&nbsp;</span>
													<i class="fa fa-times" onclick="removeAction();" title="删除动作"></i><span>&nbsp;</span>
													</sec:authorize>
													</c:if>
													...
													<i class="fa fa-plus" onclick="addAction();" title="添加动作"></i><span>&nbsp;</span>
													<i class="fa fa-times" onclick="removeAction();" title="删除动作"></i><span>&nbsp;</span>
													<i class="fa fa-anchor" onclick="viewDelegate();" title="查看委托"></i><span>&nbsp;</span>
													<i class="fa fa-twitter" onclick="toExecuteAction();" title="发送委托"></i><span>&nbsp;</span>
													<i class="fa fa-check-square-o" onclick="doneExecuteAction();" title="完成动作"></i><span>&nbsp;</span>
													<i class="fa fa-coffee" onclick="prepareDelegate();" title="预备委托"></i><span>&nbsp;</span>
										...
										size="40" minlength="2" maxlength="50" class="form-control" readonly="readonly">
										  ...
										  <tr>
											...
											<td>
												...
												vAttrId="5a486994-55d7-11e4-bdcd-a4db305e5cc5" 
                    ...
                    if(node.nodeType == 'action' && node.prime == 'T'){
						...
						<sec:authorize ifAnyGranted="ROLE_FRE-ORDER-INPUT-MANAGE-ACTION-SALE" >
							if(node.status == '已执行' || node.status == '已发送'){
	                    		...
	                    		addPrime(node.id, false, true);
	                    	}
						</sec:authorize>
						</c:if>
						
						<c:if test="${item.orderStatus == '确认追回'}">
						<sec:authorize ifAnyGranted="ROLE_FRE-ORDER-INPUT-MANAGE-ACTION-SALE,ROLE_FRE-ORDER-INPUT-MANAGE-ACTION-MANIPULATOR" >
							if(node.status == '已执行' || node.status == '已发送'){
	                    		addPrime(node.id, true, true);
	                    	}else{
	                    		addPrime(node.id, true, true);
		...
		$(function() {
			$("#freightOrderForm").validate({
		        submitHandler: function(form) {
		    		...
		    		form.submit();
		    		/**
		    		var data = toJsonString('freightOrderForm');
		    		$.ajax({
		    			type: 'POST',
		    			data: data,
		    			url: 'fre-order-save-async.do',
	...
	$(function() {
		new createListPicker({
			...
			sumitAction:'xx',//提交的action
			thead:['id','名称', '是否对内部','是否需要发送委托','是否有填报界面'],//列表头 数组
			tbody:['id', 'typeName','internal','delegate','prime'],//字段数据 数组
