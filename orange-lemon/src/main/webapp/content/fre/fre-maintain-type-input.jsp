				  ...
				  <div class="col-md-12">
				  	<div class="portlet box red">
						<div class="portlet-title">
							<div class="caption"><i class="fa fa-align-justify"></i>操作类型信息</div>
						</div>
						<div class="portlet-body form">
						<!-- BEGIN FORM-->
							<form id="editDomainForm" method="post" action="fre-maintain-type-save.do?operationMode=STORE" class="form-horizontal">
											...
											<tr>
												...
												<td>
													<select id="priority" name="priority" class="form-control required">
														<option value="1"<c:if test="${item.priority == '1'}">selected</c:if> >1</option>
														<option value="2"<c:if test="${item.priority == '2'}">selected</c:if> >2</option>
														<option value="3"<c:if test="${item.priority == '3'}">selected</c:if> >3</option>
													</select>
												</td>
												<td>
												...
												<td>
													<select id="status" name="status" class="form-control required">
														...
														<option value="F"<c:if test="${item.status == 'F'}">selected</c:if> >停用</option>
													</select>
												</td>
											</tr>
										</tbody>
    ...
    $(function() {
        $("#editDomainForm").validate({
            ...
            rules: {
            	typeName: {
   	                remote: {
   	                    url: 'fre-maintain-type-check-typename.do',
   	                    data: {
   	                        <c:if test="${item.id != null}">
   	                        id: function() {
   	                            return $('#id').val();
