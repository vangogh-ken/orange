				  ...
				  <div class="col-md-12">
				  	<div class="portlet box red">
						<div class="portlet-title">
							<div class="caption"><i class="fa fa-align-justify"></i>订单管理(销售)</div>
						</div>
						<div class="portlet-body">
							...
							<article class="m-widget" id="searchAcrticle">
								<form name="searchForm" method="post" action="fre-order-list-salesman.do" class="form-inline">
									...
									<form id="dynamicGridForm" name="dynamicGridForm" action="" class="m-form-dynamic" method="post">
										<table id="dynamicGrid" class="m-table table-striped table-bordered table-hover">
											...
											<c:forEach items="${pageView.results}" var="item">
								                <tr ondblclick="window.open('fre-order-input-salesman.do?id=${item.id}');">
			...
			<div class="modal-body">
				...
				<article class="m-widget">
					<form id="expenseBtnForm" name="expenseBtnForm" method="post" action="" class="form-inline" onsubmit="return false;">
						...
						<button type="button" class="btn btn-sm green" onclick="toExpenseAudit();">提交审核</button>
				...
				<!-- style="max-height: 300px;overflow-y: scroll;" style="display: none" -->
				<!-- 已有费用 -->
				<article class="m-widget" >
...
//~~~~~~~~~~~~~~~~~~~~~~~~~~~~~费用
			...
			if(expenseStatus != '未添加' && expenseStatus != '添加中'){
				...
				$.ajax({
					...
					success:function(data){
						...
						$.each(hasAddData, function(i, item){
							if(item.incomeOrExpense == '收'){
								...
								if(item.predictAmount != 0){
									...
									html += '<td style="background-color:red;">' + item.predictAmount + '</td>';
								}
								html += '<td>' + item.taxation + '</td>';
						...
						if(countIncome>0 || countExpense>0){
							html += '<tr><td colspan="2"></td><td>共'+(countIncome + countExpense)+'条</td><td colspan="6"></td><td>总计:'+freightOrder.balance+'</td><td>小计:'+(Math.round((freightOrder.paymentTax - freightOrder.incomeTax)*100)/100)+'</td><td></td><td></td><td></td></tr>';
						}
	...
	function reviseExpense(){
		if($('#expenseHasAddForm .selectedItemId:checked').length != 1){
			...
			if(status != '未提交' && status != '审核中' && status != '已退回' && status != '已审核' && status != '已对账'){
		...
		if($('.selectedItem:checked').length != 1){
			...
			$.ajax({
				...
				success:function(data){
					...
					html += '<thead><tr><th>起始地</th><th>终止地</th><th>集装箱来源</th><th>箱型</th><th>箱属</th><th>箱况</th><th>数量</th><th>提单号</th></tr></thead><tbody>';
	...
	function toAddOrderByCopy(){
		if($('.selectedItem:checked').length != 1){
			...
			$.post('fre-order-to-add-order-by-copy.do?freightOrderId=' + freightOrderId,function(data){
				...
				html += '<th>起始地</th><th>目的地</th><th>箱型</th><th>箱属</th><th>箱况</th><th>数量</th><th>说明</th></tr></thead><tbody>';
	        ...
	        submitHandler: function(form) {
				...
				$.post('fre-order-done-add-order-by-copy.do', $("#toAddOrderByCopyForm").serialize(), function(data){
					if(data != null && data != 'error'){
						box.modal('hide');
						window.open('../fre/fre-order-input.do?id=' + data);
					}
				});
	        },
	        errorClass: 'validate-error'
