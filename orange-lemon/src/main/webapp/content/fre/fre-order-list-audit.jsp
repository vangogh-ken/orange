				  ...
				  <div class="col-md-12">
				  	<div class="portlet box red">
						<div class="portlet-title">
							<div class="caption"><i class="fa fa-align-justify"></i>订单管理(审核)</div>
						</div>
						<div class="portlet-body">
							<article class="m-widget" id="btnAcrticle">
								<div class="pull-left">
										...
										<button class="btn btn-sm red" id="doneExpenseAudit">审核费用</button>
										<button class="btn btn-sm red" id="backExpenseAudit">退回费用</button>
										</sec:authorize>
										
										<!-- 对于亏损订单的三级审核 -->
										<sec:authorize ifAnyGranted="ROLE_CY-MARKETING">
										<button class="btn btn-sm green" id="doneRehearOrder">复审订单</button>
										<button class="btn btn-sm green" id="backRehearOrder">复审退回</button>
										</sec:authorize>
										
										<sec:authorize ifAnyGranted="ROLE_CY-GENERAL-MANAGER-EMP">
										<button class="btn btn-sm red" id="doneEventideOrder">终审订单</button>
										<button class="btn btn-sm red" id="backEventideOrder">终审退回</button>
										</sec:authorize>
										
										<sec:authorize ifAnyGranted="ROLE_FRE-ORDER-LIST-BOX-REQUIRE-VIEW"></sec:authorize>
										<button class="btn btn-sm green" id="viewRequire">
											箱需详情</button>
										
										
										<sec:authorize ifAnyGranted="ROLE_FRE-ORDER-LIST-ORDER-BOX-VIEW"></sec:authorize>
										<button class="btn btn-sm green" id="viewOrderBox">
											箱封详情</button>
										
										
										<sec:authorize ifAnyGranted="ROLE_FRE-ORDER-LIST-EXPENSE-VIEW"></sec:authorize>
							...
							<article class="m-widget" id="searchAcrticle">
								<form name="searchForm" method="post" action="fre-order-list-audit.do" class="form-inline">
								    ...
								    <select id="orderStatus" name="orderStatus" class="form-control">
								    	...
								    	<option value="订单初审中" <c:if test="${param.orderStatus == '订单初审中'}">selected</c:if> >订单初审中</option>
								    	<option value="订单复审中" <c:if test="${param.orderStatus == '订单复审中'}">selected</c:if> >订单复审中</option>
								    	<option value="订单终审中" <c:if test="${param.orderStatus == '订单终审中'}">selected</c:if> >订单终审中</option>
	...
	$(document).delegate('#doneOrderAudit', 'click',function(e){
		if($('.selectedItem:checked').length == 0){
			...
			$('.selectedItem:checked').each(function(i, item){
				...
				if(orderStatus != '审核中' && orderStatus != '订单初审中' && orderStatus != '锁定中' ){
					flag = false;
				}else{
					if(i == 0){
...
//~~~~~~~~~~~~~~~~~~~~~~~~~~~~~费用
	...
	//订单复审
	$(document).delegate('#doneRehearOrder', 'click',function(e){
		if($('.selectedItem:checked').length == 0){
			alert('请选择数据!');
			return;
		}else{
			var url = 'fre-order-done-rehear-order.do?';
			var flag = true;
			$('.selectedItem:checked').each(function(i, item){
				var orderStatus = $('#' + $(item).val() + 'orderStatus').text();
				if(orderStatus != '订单复审中'){
					flag = false;
				}else{
					if(i == 0){
						url += 'freightOrderId=' +　$(item).val();
			...
			if(flag){
				...
				alert('复审失败！请确认所选订单状态！');
				return;
			}
		}
	});
	
	$(document).delegate('#backRehearOrder', 'click',function(e){
		if($('.selectedItem:checked').length == 0){
			alert('请选择数据!');
			return;
		}else{
			var url = 'fre-order-back-rehear-order.do?';
			var flag = true;
			$('.selectedItem:checked').each(function(i, item){
				var orderStatus = $('#' + $(item).val() + 'orderStatus').text();
				if(orderStatus != '订单复审中'){
	...
	//订单终审
	$(document).delegate('#doneEventideOrder', 'click',function(e){
		if($('.selectedItem:checked').length == 0){
			alert('请选择数据!');
			return;
		}else{
			var url = 'fre-order-done-eventide-order.do?';
			var flag = true;
			$('.selectedItem:checked').each(function(i, item){
				var orderStatus = $('#' + $(item).val() + 'orderStatus').text();
				if(orderStatus != '订单终审中'){
					flag = false;
				}else{
					if(i == 0){
						url += 'freightOrderId=' +　$(item).val();
	...
	$(document).delegate('#backEventideOrder', 'click',function(e){
		if($('.selectedItem:checked').length == 0){
			alert('请选择数据!');
			return;
		}else{
			var url = 'fre-order-back-eventide-order.do?';
			var flag = true;
			$('.selectedItem:checked').each(function(i, item){
				var orderStatus = $('#' + $(item).val() + 'orderStatus').text();
				if(orderStatus != '订单终审中'){
