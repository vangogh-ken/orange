				  ...
				  <div class="col-md-12">
				  	<div class="portlet box red">
						<div class="portlet-title">
							<div class="caption"><i class="fa fa-align-justify"></i>放箱管理(箱管)</div>
						</div>
						<div class="portlet-body">
							...
							<article class="m-widget" id="searchAcrticle">
								<form name="searchForm" method="post" action="fre-box-require-list-release.do" class="form-inline" method="post">
	...
	$(document).delegate('#viewOrderBox', 'click', function(e){
		if($('.selectedItem:checked').length != 1){
			...
			listData(url, '箱封明细', 1000,
					['起始地','终止地', '箱型','箱属','集装箱号','封号', '备注(提箱地)', '状态'], 
					['freightBoxRequire.beginStation','freightBoxRequire.arriveStation','freightBox.boxType', 'freightBox.boxBelong', 'freightBox.boxNumber', 'freightSeal.sealNumber', 'descn','freightBoxRequire.status']
			);
		}
	});
...
//////////////////////////选箱配封	
		...
		if($('.selectedItem:checked').length != 1){
			...
			if(true){
				$.post('fre-box-require-to-choose-boxseal.do?freightBoxRequireId=' + freightBoxRequireId, function(data){
					...
					html += '<table class="m-table table-bordered table-hover">';
					html += '<thead><tr>';
					html += '<th>序号</th><th>箱型</th><th>箱属</th><th>箱况</th><th>集装箱来源</th><th>箱号</th><th>封号</th><th>提箱地</th></tr></thead><tbody>';
					$.each(freightOrderBoxs, function(i, item){
						...
						if(item.freightSeal != null){
							html += '<td><input id="'+ item.id +'sealNumber" name="sealNumber" value="' + item.freightSeal.sealNumber + '" class="form-control"></td>';
						}else{
							html += '<td><input id="'+ item.id +'sealNumber" name="sealNumber" class="form-control"></td>';
						}
						
						html += '<td><input id="'+ item.id +'location" name="location" value="'+ (item.location == null ? '' : item.location) +'" vAttrId="ce4e5129-9845-11e5-8639-b870f47f73d5" class="form-control required dictionary"></td></tr>';
					});
					html += '</tbody></table>';
	...
	/* function validateNumber(){
		...
		$('#chooseBoxSealForm input[name=boxNumber]').each(function(i, item){
			if(flag){
				if($(item).val() == ''){
					$(item).after('<span class="validate-error">为空或校验失败</span>');
					flag = false;
				}else{
					$('.validate-error').remove();
					$('#chooseBoxSealForm input[name=boxNumber]').each(function(n, ele){
						if(i != n && $(item).val() == $(ele).val()){
							$(item).after('<span class="validate-error">为空或校验失败</span>');
							flag = false;
						}
					});
		...
		$('#chooseBoxSealForm input[name=sealNumber]').each(function(i, item){
			if(flag){
				$('#chooseBoxSealForm span .validate-error').remove();
				$('#chooseBoxSealForm input[name=sealNumber]').each(function(n, ele){
					if(i != n && $(item).val() != '' && $(ele).val() != '' && $(item).val() == $(ele).val()){
						$(item).after('<span class="validate-error">为空或校验失败</span>');
						flag = false;
					}
				});
		...
		if(flag){
			flag = $('#chooseBoxSealForm').find('.validate-error').length > 0 ? false : true;
		}
		alert(flag);
		flag = false;
		return flag;
