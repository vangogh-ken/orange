  ...
  <body class="page-header-fixed">
				  ...
				  <div class="col-md-12">
				  	<div class="portlet box red">
						<div class="portlet-title">
							<div class="caption"><i class="fa fa-anchor"></i>委托管理(个人级)</div>
						</div>
						<div class="portlet-body">
							...
							<article class="m-widget" id="searchAcrticle">
								<form name="searchForm" method="post" action="fre-delegate-list-owner.do" class="form-inline">
								    ...
								    <input type="text" id="typeName" name="typeName" value="${param.typeName}" class="form-control input-small">
								    
								    <label for="manipulator">操作员</label>
								    <input type="text" id="manipulator" name="manipulator" value="${param.manipulator}" class="form-control input-small">
							...
							<article class="m-widget">
									<form id="dynamicGridForm" name="dynamicGridForm" action="fre-delegate-remove-owner.do" class="m-form-dynamic" method="post">
											...
											<c:forEach items="${pageView.results}" var="item">
								                <tr>
								                    ...
								                    <td>
									                    ...
									                    <!--  
									                    <a href="/VC/convert?fileName=${item.delegateFile}" target="_blank" onclick="browseDelegate('${item.freightAction.id}');">预览</a>
									                    -->
			...
			<div class="modal-body">
				...
				<article class="m-widget">
					<form id="expenseBtnForm" name="expenseBtnForm" method="post" action="" class="form-inline" onsubmit="return false;">
						...
						<button type="button" class="btn btn-sm red" onclick="reviseExpense();" id="reviseExpenseBtn">修改</button>
		...
		if($('.selectedItem:checked').length != 1){
			...
			$.post('fre-delegate-by-delegateid.do?freightDelegateId=' + freightDelegateId, function(data){
				...
				if(freightAction.internal == 'T' && 
						('待执行' == freightDelegate.status || '已执行' == freightDelegate.status)){
					$('#finishExpenseBtn').hide();//单独修改费用，则隐藏该按钮
					addExpense(freightAction.id);
				}else{
					alert('非对内委托或非待执行和已执行状态，不能处理费用！');
				}
			});
		}
	});
	...
	function reviseExpense(){
		if($('#expenseHasAddForm .selectedItemId:checked').length != 1){
			...
			var status = $('#' + $('#expenseHasAddForm .selectedItemId:checked').val() + 'status').text();
			if(status == '已对账'){
				alert(status + '的费用不能修改！');
			}else{
