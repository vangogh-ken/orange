...
<html lang="en">
  ...
  <body class="page-header-fixed">
								 ...
								 <div class="form-body">
									<table class="table-input" style="margin-left: 10%;margin-right: 10%;width: 80%;">
										...
										<tbody>
											<tr>
												<td>
													<label class="td-label" for="crmCustomerId">单位名称</label>
												</td>
												<td>
												...
												<td>
													<input id="contactPerson" type="text" name="contactPerson" value="${lastFollow != null ? lastFollow.contactPerson : item.crmCustomer.contactPerson}" 
											...
											<tr>
												...
												<td>
													<input id="contactPosition" type="text" name="contactPosition" value="${lastFollow != null ? lastFollow.contactPosition : item.crmCustomer.contactPosition}" 
													size="40" minlength="2" maxlength="50" class="form-control required" >
												</td>
												<td>
												...
												<td>
													<input id="contactPhone" type="text" name="contactPhone" value="${lastFollow != null ? lastFollow.contactPhone : item.crmCustomer.contactPhone}" 
													size="40" minlength="2" maxlength="50" class="form-control required" >
												</td>
											</tr>
											<tr>
											...
											<tr>
												...
												<td>
													<input id="lastFollowTime" type="text" name="lastFollowTime" value="<fmt:formatDate value="${lastFollow != null ? lastFollow.currentFollowTime : (item.lastFollowTime == null ? now : item.lastFollowTime)}" type="both" dateStyle="long" pattern="yyyy-MM-dd" />" 
													size="40" minlength="2" maxlength="50" class="form-control required datepicker" >
												</td>
												<td>
												...
												<td>
													<input id="nextFollowTime" type="text" name="nextFollowTime" value="${item.nextFollowTime}" 
													size="40" minlength="2" maxlength="50" class="form-control required datepicker" >
												</td>
											</tr>
											<tr>
											...
											<tr>
												...
												<td colspan="3">
													<textarea id="chanceSuggest" name="chanceSuggest" class="form-control required" style="min-height:80px;" <c:if test="${item.status == null || item.status != '未提交'}">readonly="readonly"</c:if> >${item.descn == null ? '无' : item.chanceSuggest}</textarea>
												</td>
											</tr>
										</tbody>
									</table>
    ...
    $(document).delegate('#currentFollowTime', 'change', function(event){
    	var currentFollowTime = new Date($(this).val().replace('-','/').replace('-','/'));
    	//7天之前的不能录入
    	if(new Date().getTime() - currentFollowTime.getTime() > 1000*60*60*24*7 || new Date().getTime() - currentFollowTime.getTime() < 0){
    		alert('不能录入7天之前或今天之后的拜访记录！');
    		$(this).val('');
    	}
    });
