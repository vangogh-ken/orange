...
<html lang="en">
  ...
  <body class="page-header-fixed">
				...
				<div class="row">
				  <div class="col-md-12">
				  	<div class="portlet box red">
						...
						<!-- BEGIN FORM-->
							<form id="businessInterfaceSchemaForm" method="post" action="carinfo-save.do?operationMode=STORE" class="form-horizontal">
								...
								<div class="form-body">
										...
										<div class="col-md-4">
											<select name="clsId" id="clsId" class="form-control required" >
											  	<c:forEach items="${clses}" var="cls">
											  		<option value="${cls.id}" <c:if test="${item.businessClass.id eq cls.id}">selected="selected"</c:if>>${cls.name}</option>
											  	</c:forEach>
											  </select>
										</div>
									</div>
									...
									<div class="form-group">
										<label class="control-label col-md-3">数据过滤</label>
										<div class="col-md-4">
											<input id="filter" name="filter" type="text" value="${item.filter}" class="form-control required"/>
										</div>
									</div>
    ...
    $(function() {
        $("#businessInterfaceSchemaForm").validate({
