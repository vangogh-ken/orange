...
<html lang="en">
  ...
  <body class="page-header-fixed">
						...
						<div class="portlet-body">
							<article class="m-widget" id="btnAcrticle">
								<div class="pull-left">
									...
									<button class="btn btn-sm yellow" onclick="window.location.href = 'disk-share-list.do?display=list&diskDir=${diskDir}'" >
										<li class="fa fa-list"></li>
									</button>
									</c:if>
									<c:if test="${display == 'list'}">
									<button class="btn btn-sm yellow" onclick="window.location.href = 'disk-share-list.do?display=grid&diskDir=${diskDir}'" >
							...
							<article class="m-widget">
								...
								<div class="row">
								  <c:forEach items="${pageView.results}" var="item" varStatus="status">
									  <div class="col-md-2 text-center">
									    <c:if test="${item.diskInfo.fileSuffix == 'dir'}">
									    <a href="disk-info-list.do?diskDir=${diskDir}/${item.diskInfo.fileName}&display=${display}"><div class="icon-62 icon-62-${item.diskInfo.fileSuffix}"></div></a>
									    <a href="disk-info-list.do?diskDir=${diskDir}/${item.diskInfo.fileName}&display=${display}"><div class="file-62-name">${item.diskInfo.fileName}</div></a>
										</c:if>
									    <c:if test="${item.diskInfo.fileSuffix != 'dir'}">
									    <!--  
									    ...
									    -->
									    <a href="javascript:void(0);toViewDisk('${item.diskInfo.id}');" click="toViewDisk('${item.diskInfo.id}');"><div class="icon-62 icon-62-${item.diskInfo.fileSuffix}"></div></a>
									    <a href="javascript:void(0);toViewDisk('${item.diskInfo.id}');" click="toViewDisk('${item.diskInfo.id}')"><div class="file-62-name">${item.diskInfo.fileName}</div></a>
										</c:if>
									  </div>
								...
								<c:if test="${display == null || display == 'list'}">
									...
									<table id="dynamicGrid" class="m-table table-striped table-bordered table-hover">
										<thead>
									      <tr>
									        ...
									        <th style="min-width:220px;">文件名</th>
									        <th>大小</th>
									    ...
									    <tbody>
									      ...
									      <tr>
									        <td><input type="checkbox" class="selectedItem a-check" name="selectedItem" value="${item.id}" valued="${item.diskInfo.id}"></td>
									        <td>
									        	<c:if test="${item.diskInfo.fileSuffix == 'dir'}">
											    <a href="disk-info-list.do?diskDir=${diskDir}/${item.diskInfo.fileName}&display=${display}"><div class="icon-16 icon-16-${item.diskInfo.fileSuffix}">&nbsp;${item.diskInfo.fileName}</div></a>
												</c:if>
												
											    <c:if test="${item.diskInfo.fileSuffix != 'dir'}">
											    <a href="javascript:void(0);" onclick="toViewDisk('${item.diskInfo.id}')"><div class="icon-16 icon-16-${item.diskInfo.fileSuffix}">&nbsp;${item.diskInfo.fileName}</div></a>
											    </c:if>
											</td>
									        <td>
									        	${item.diskInfo.fileSuffix == 'dir' ? '' : item.diskInfo.fileSize <= 1024 ? item.diskInfo.fileSize : 
									        	item.diskInfo.fileSize <= 1024*1024 ? (item.diskInfo.fileSize - item.diskInfo.fileSize%1024 + 1024)/1024 : 
									        	item.diskInfo.fileSize <= 1024*1024*1024 ? (item.diskInfo.fileSize - item.diskInfo.fileSize%(1024*1024) + 1024*1024)/(1024*1024) : 
									        	item.diskInfo.fileSize <= 1024*1024*1024*1024 ? (item.diskInfo.fileSize - item.diskInfo.fileSize%(1024*1024*1024) + 1024*1024*1024)/(1024*1024*1024) : 
									        	(item.diskInfo.fileSize - item.diskInfo.fileSize%(1024*1024*1024*1024) + 1024*1024*1024*1024)/(1024*1024*1024*1024)}
									        	
									        	${item.diskInfo.fileSuffix == 'dir' ? '' : item.diskInfo.fileSize <= 1024 ? 'B' : 
									        	item.diskInfo.fileSize <= 1024*1024 ? 'KB' : 
									        	item.diskInfo.fileSize <= 1024*1024*1024 ? 'MB' : 
									        	item.diskInfo.fileSize <= 1024*1024*1024*1024 ? 'GB' : 'TB'}
									        </td>
									        <td><fmt:formatDate value="${item.createTime}" pattern="yyyy-MM-dd HH:mm:ss"/></td>
									        <td><fmt:formatDate value="${item.modifyTime}" pattern="yyyy-MM-dd HH:mm:ss"/></td>
									        <td>
											    <c:if test="${userSession.id == item.diskInfo.creator.id}">
											    <a href="javascript:void(0);" onclick="recallShareDiskByDiskShareId('${item.id}')" >取消分享</a>
												</c:if>
									        </td>
									      </tr>
									      </c:forEach>
	...
	$(document).delegate('#doneDownLoadDisk', 'click', function(){
		if($('.selectedItem:checked').length > 0){
			...
			$('.selectedItem:checked').each(function(i, item){
				url += 'diskInfoId=' + $(item).attr('valued') + '&';
			});
	...
	**/
	...
	//取消分享
	function recallShareDiskByDiskShareId(diskShareId){
		$.post('disk-info-recall-share-disk-by-disk-share-id.do?diskShareId=' + diskShareId, function(data){
			if(data == 'success'){
