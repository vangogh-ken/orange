				...
				<div class="row">
				  <div class="col-md-12">
				  	<div class="portlet box red">
						...
						<!-- BEGIN FORM-->
							<form id="editDomainForm" method="post" action="rep-template-save-statistics.do?operationMode=STORE" enctype="multipart/form-data" class="form-horizontal">
									...
									<table class="table-input" style="margin-left:10%;width:80%">
										...
										<tbody>
											...
											<tr>
												...
												<td>
													<select id="templateType" name="templateType" class="form-control required">
														<option value="报表" <c:if test="${item.templateType == '报表' }">selected</c:if>>报表</option>
													</select>
												</td>
												<td>
											...
											<tr>
												...
												<td>
													<input id="muiltFile" name="muiltFile" type="file" class="form-control"/>
												</td>
												<td>
													<label class="td-label" for="graphUrl">图表页面</label>
												</td>
												<td>
													<input id="graphUrl" type="text" name="graphUrl" 
													value="${item.graphUrl}" 
													size="40" minlength="2" maxlength="64" class="form-control" >
												</td>
												<!--  
												<td>
											...
											<tr>
												<td>
													<label class="td-label" for="beanClass">执行类型</label>
												</td>
												<td>
													<input id="beanClass" type="text" name="beanClass" 
													value="${item.beanClass}" 
													size="40" minlength="2" maxlength="128" class="form-control" >
												</td>
												<td>
    ...
    $(function() {
        $("#editDomainForm").validate({
            ...
            rules: {
            	templateName: {
   	                remote: {
   	                    url: 'rep-template-check-templatename.do',
   	                    data: {
   	                        <c:if test="${item.id != null}">
   	                        id: function() {
   	                            return $('#id').val();
